...
const resolvers = {
  ...
  Mutation: {
    ...
    signInWithGoogle: async (parent, args) => {
      ...
      // const [r] = await dbPromisePool.query(`insert into users (username, password, name, create_at, phone, email, active) values (?, ?, ?, ?, ?, ?, '1')`, [uuidv4(), googleTokenPassword, String(payload.name), Math.floor(Date.now() / 1000), null, String(payload.email)])
    ...
    jsFiddle: async (parent, args) => {
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          returns.output = ret;
          `)
          resolve(util.format(returns.output))
        } catch (e) {
          reject(e)
        }
      })
    ...
    mp3tag: async (parent, args) => {
      ...
      for (const apic of mp3tag.tags?.v2?.APIC || []) {
        const coverpath = 'files/' + uuidv4() + '.jpg'
